...
class HtmlLorem extends Base
{
    ...
    private function addRandomAttribute(\DOMElement $node): void
    {
        ...
        switch ($rand) {
            case 1:
                $node->setAttribute('class', $this->generator->callAnonymous('word'));

                break;

            case 2:
                $node->setAttribute('id', (string) static::randomNumber(5));
    ...
    private function addRandomP(\DOMElement $element, $maxLength = 10): void
    {
        ...
        $node->textContent = $this->generator->callAnonymous('sentence', [self::numberBetween(1, $maxLength)]);
        $element->appendChild($node);
    }

    private function addRandomText(\DOMElement $element, $maxLength = 10): void
    {
        $text = $element->ownerDocument->createTextNode($this->generator->callAnonymous('sentence', [self::numberBetween(1, $maxLength)]));
    ...
    private function addRandomA(\DOMElement $element, $maxLength = 10): void
    {
        ...
        $node->setAttribute('href', $this->generator->callAnonymous('safeEmailDomain'));
    ...
    private function addLoginForm(\DOMElement $element): void
    {
        ...
        $textLabel->textContent = $this->generator->callAnonymous('word');
        ...
        $passwordLabel->textContent = $this->generator->callAnonymous('word');

        $submit = $element->ownerDocument->createElement(static::INPUT_TAG);
        $submit->setAttribute('type', 'submit');
        $submit->setAttribute('value', $this->generator->callAnonymous('word'));

        $submit = $element->ownerDocument->createElement(static::FORM_TAG);
        $submit->setAttribute('action', $this->generator->callAnonymous('safeEmailDomain'));
    ...
    private function addRandomTable(\DOMElement $element, $maxRows = 10, $maxCols = 6, $maxTitle = 4, $maxLength = 10): void
    {
        ...
        for ($i = 0; $i < $cols; ++$i) {
            ...
            $th->textContent = $this->generator->callAnonymous('sentence', [self::numberBetween(1, $maxTitle)]);
            $tr->appendChild($th);
        }
        ...
        for ($i = 0; $i < $rows; ++$i) {
            ...
            for ($j = 0; $j < $cols; ++$j) {
                ...
                $th->textContent = $this->generator->callAnonymous('sentence', [self::numberBetween(1, $maxLength)]);
                $tr->appendChild($th);
            }
        }
    ...
    private function addRandomUL(\DOMElement $element, $maxItems = 11, $maxLength = 4): void
    {
        ...
        for ($i = 0; $i < $num; ++$i) {
            ...
            $li->textContent = $this->generator->callAnonymous('sentence', [self::numberBetween(1, $maxLength)]);
            $ul->appendChild($li);
        }
